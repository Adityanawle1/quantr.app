    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-border-subtle pb-6">
        <div>
          ...
          <p className="text-t3 mt-1.5 font-medium">Deploy pre-built quantitative strategies or browse the market.</p>
        </div>
          ...
          {SCREENS.map(s => {
            ...
            return (
              <div
                ...
                className={`p-4 rounded-[12px] border cursor-pointer transition-all duration-300 group flex flex-col items-start gap-4 ${isActive ? 'bg-[rgba(37,99,235,0.06)] border-[rgba(37,99,235,0.35)] shadow-[0_0_0_4px_rgba(37,99,235,0.08)] scale-[0.98]' : 'bg-background-primary border-border-subtle hover:bg-accent-blue-muted hover:border-[rgba(37,99,235,0.2)]'}`}
              >
                <div className={`text-[11px] font-bold px-2 py-1 rounded-[6px] border transition-colors ${isActive ? 'bg-primary text-white border-primary' : 'text-t2 border-border-subtle bg-background-elevated group-hover:text-primary group-hover:border-primary/30'}`}>
                  {s.badge}
                </div>
                <div>
                  <h3 className={`font-display text-[16px] font-bold mb-1 transition-colors ${isActive ? 'text-primary' : 'text-t1'}`}>{s.label}</h3>
      ...
      {/* Tools & Table View */}
      <div className="bg-background-primary border border-border-subtle rounded-[20px] overflow-hidden flex flex-col shadow-sm">
        {/* Table Toolbar */}
        <div className="px-5 py-4 border-b border-border-subtle flex items-center justify-between bg-background-surface">
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            <span className="font-mono text-xs text-t3 tracking-widest">{stocks.length} EQUITIES FOUND</span>
          </div>
          ...
          <select
            ...
            className="bg-background-primary border border-border-default text-xs text-t1 rounded-lg px-3 py-1.5 outline-none focus:border-primary/40 focus:ring-1 focus:ring-primary/20 transition-shadow"
          >
        ...
        <div className="overflow-x-auto min-h-[400px]">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-[11px] font-bold text-t3 uppercase tracking-wider border-b border-border-subtle bg-background-surface">
                ...
                ? Array.from({ length: 8 }).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    {Array.from({ length: 8 }).map((_, j) => (
                      <td key={j} className="px-6 py-5"><div className="h-4 bg-background-elevated rounded w-16" /></td>
                    ))}
                  </tr>
                ))
                : stocks.map((s: any) => (
                  <tr
                    ...
                    className="hover:bg-accent-blue-muted cursor-pointer transition-colors group"
                  >
                    <td className="px-6 py-4 font-bold font-mono text-t1 group-hover:text-primary transition-colors">{s.symbol}</td>
                    <td className="px-6 py-4 text-t3 max-w-[200px] truncate">{s.name}</td>
                    ...
                    <td className="px-6 py-4 font-mono text-right text-t3">{s.marketCapFormatted || '-'}</td>
                    <td className="px-6 py-4 font-mono text-right text-t3">{s.pe?.toFixed(1) || '-'}</td>
                    <td className="px-6 py-4 font-mono text-right text-t3">{s.roe ? `${s.roe}%` : '-'}</td>
                    <td className="px-6 py-4 font-mono text-right text-t3">{s.debtToEquity?.toFixed(2) || '-'}</td>
                  </tr>
                ))}
